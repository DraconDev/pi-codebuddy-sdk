...
import { attachMcpHandler, claimMcpToolCall, registerMcpToolCall, resolvePendingMcpHandlers } from "./mcp-handler-bridge.js";
...
// Handlers may be invoked before their tool_use stream event; the bridge binds
// them by tool name first and then matches results by the real tool-call ID.
	...
	const mcpTools = tools.map((tool) => ({
		...
		handler: async () => {
			return new Promise<McpResult>((resolve) => {
				const pending = { toolName: tool.name, resolve };
				const toolCallId = claimMcpToolCall(queryCtx, tool.name);
				if (toolCallId) {
					debug(`mcp handler: ${tool.name} [${toolCallId}] → bound existing tool call`);
					attachMcpHandler(queryCtx, toolCallId, pending, debug);
				} else {
					queryCtx.unboundToolHandlers.push(pending);
					debug(`mcp handler: ${tool.name} [pending] → waiting for tool_use event`);
				}
			});
		},
	}));
...
const REASONING_TO_EFFORT: Record<string, Effort> = {
	...
	// Agent SDK 0.3's declaration predates the runtime-supported max value.
	max: "max" as Effort,
};
...
function processStreamEvent(
	...
	if (event?.type === "content_block_start") {
		...
		if (event.content_block?.type === "text") {
			...
			const toolName = mapToolName(event.content_block.name, customToolNameToPi);
			registerMcpToolCall(c, event.content_block.id, toolName, debug);
			c.turnBlocks.push({
...
function processAssistantMessage(message: CbMessage, model: Model<any>, customToolNameToPi: Map<string, string>, c: QueryContext): void {
	...
	for (const block of assistantMsg.content) {
		if (block.type === "text" && block.text) {
			...
			ensureTurnStarted(c);
			c.turnSawToolCall = true;
			const toolName = mapToolName(block.name, customToolNameToPi);
			registerMcpToolCall(c, block.id, toolName, debug);
			const mappedArgs = mapToolArgs(toolName, block.input);
			c.turnBlocks.push({
...
function streamCodebuddySdk(model: Model<any>, context: Context, options?: SimpleStreamOptions): AssistantMessageEventStream {
	...
	if (resultCtx) {
		...
		debug(`provider: tool results, ${allResults.length} results, ${resultCtx.pendingToolCalls.size + resultCtx.unboundToolHandlers.length} waiting handlers, ctx.msgs=${context.messages.length}`);
		...
		if (waitingHandlers > 0) {
			debug(`WARNING: ${waitingHandlers} MCP handlers still waiting after delivering ${allResults.length} results (bound=${resultCtx.pendingToolCalls.size}, unbound=${resultCtx.unboundToolHandlers.length})`);
			piUI?.notify(`CodeBuddy SDK: ${waitingHandlers} tool handler(s) still waiting — provider may be stuck`, "warning");
		}
	...
	queryCtx.turnToolCallNames = [];
	queryCtx.turnToolCallClaimed = [];
	...
	const onAbort = () => {
		...
		resolvePendingMcpHandlers(abortCtx, "Operation aborted");
		requestAbort();
	};
	if (options?.signal) {
		if (options.signal.aborted) onAbort();
	...
	void (async () => {
		...
		try {
			...
			if (!isReentrant) {
				resolvePendingMcpHandlers(queryCtx, "Query ended");
				queryCtx.activeQuery = null;
			}
			const errStream = queryCtx.currentPiStream;
...
async function discoverModels(pi: ExtensionAPI): Promise<void> {
	await withSdkGate(async () => {
		try {
			...
			const discovered = rawModelsFromSdk(supported as any);
			// supportedModels() is a CLI catalog and can omit models enabled for
			// the account (notably hy4-preview). Preserve known fallbacks during
			// discovery so an otherwise successful catalog refresh cannot hide them.
			const discoveredIds = new Set(discovered.map((model) => model.id.toLowerCase()));
			const fallbacks = FALLBACK_MODELS.filter((model) => !discoveredIds.has(model.id.toLowerCase()));
			MODELS = buildModels(
