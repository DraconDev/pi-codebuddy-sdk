...
export class QueryContext {
	...
	// CodeBuddy can invoke an MCP handler before it emits the corresponding
	// tool_use stream event. Keep names and claimed flags alongside IDs so the
	// handler can be bound when the event arrives instead of being keyed by
	// undefined.
	turnToolCallNames: string[] = [];
	turnToolCallClaimed: boolean[] = [];
	unboundToolHandlers: PendingToolCall[] = [];
	...
	resetTurnState(model: Model<any>): void {
		...
		// Tool-call IDs, names, and claims are reset at message_start (or by the
		// assistant-message fallback) and persist across tool-result delivery
