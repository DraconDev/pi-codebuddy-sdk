...
```bash
pi --model codebuddy/hy4-preview --thinking max
```
...
This fork keeps `hy4-preview` available even when the CodeBuddy CLI's model
catalog omits it, and maps Pi's highest reasoning level (`max`) to CodeBuddy's
runtime `max` effort. The model is account-dependent; if it is not enabled,
choose another `codebuddy/...` model.

Tools, skills, extensions, `/compact`, and steer behave the same as with other Pi providers.

## Tool-call race fix

The CodeBuddy SDK can start MCP tool handlers before it emits the stream event
that contains each tool-call ID. The bridge now queues those handlers by tool
name, binds them when the event arrives, and matches later Pi results by the
real ID. This prevents the `1 tool handler(s) still waiting — provider may be
stuck` warning and supports multiple calls in the same turn.
...
even when SDK discovery is still in flight (new process, gate busy, or stale auth). The
fork also adds `hy4-preview` as a fallback. If the model is still missing, share the debug
log (`CODEBUDDY_SDK_DEBUG=1`) and the cache file.

**A tool call is stuck or the provider warning appears?**
Restart the current Pi process after installing the fork. Then enable
`CODEBUDDY_SDK_DEBUG=1` and inspect `~/.pi/agent/codebuddy-sdk.log`; successful
binding includes `mcp tool call: ... → bound waiting handler` followed by a
`provider: resolving ...` line.
