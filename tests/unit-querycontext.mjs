...
describe("QueryContext class", () => {
	...
	it("resetTurnState preserves tool-call bindings", () => {
		ctx().turnToolCallIds = ["id1", "id2"];
		ctx().turnToolCallNames = ["bash", "read"];
		ctx().turnToolCallClaimed = [true, false];
		ctx().resetTurnState(fakeModel);

		assert.deepStrictEqual(ctx().turnToolCallIds, ["id1", "id2"]);
		assert.deepStrictEqual(ctx().turnToolCallNames, ["bash", "read"]);
		assert.deepStrictEqual(ctx().turnToolCallClaimed, [true, false]);
	});
});
